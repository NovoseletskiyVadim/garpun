...
const setBotUserList = (userList: string | undefined) => {
    if (userList) {
...
const appConfig = {
    ...
    MAIN_DB_PATH,
    TEMP_DB_PATH,
    TIMEOUT_CAMERA_OFF_ALERT: TIMEOUT_CAMERA_OFF_ALERT ? parseInt(TIMEOUT_CAMERA_OFF_ALERT, 10) : 3600000,
    ARCHIVE_DAYS: ARCHIVE_DAYS ? parseInt(ARCHIVE_DAYS, 10) : 0,
    TRASH_ARCHIVE_DAYS: TRASH_ARCHIVE_DAYS ? parseInt(TRASH_ARCHIVE_DAYS, 10) : 0,
    MAX_FILE_SIZE: MAX_FILE_SIZE ? parseInt(MAX_FILE_SIZE, 10) : 250000,
    BOT_TOKEN,
    MAX_REQUESTS_COUNT: MAX_REQUESTS_COUNT ? parseInt(MAX_REQUESTS_COUNT, 10) : 50,
    REQUEST_TIMEOUT: REQUEST_TIMEOUT ? parseInt(REQUEST_TIMEOUT, 10) : 1000,

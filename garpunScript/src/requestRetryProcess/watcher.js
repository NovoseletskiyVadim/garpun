...
const { resender } = require('./resender');
/**
 * @Class RejectWatcher
            ...
            this.jsonResend(this.limit, this.countAttempt)
                .then((result) => {
                    ...
                    if (sentList && sentList.length) {
                        ...
                        const percentageOfDelivered =                            (100 * calcResult.fulfilled) / sentList.length;
                    ...
                    if (count !== null && this.alertsHistory.lastCount !== count) {
                        const { isGrown, shouldSent } =                            this.isShouldSendToBot(count);
                        if (shouldSent) {
                            if (count > 0) {
                                ...
                                appLogger.setLogMessage(botMessage).botMessage(` ${botIcon}`);
                            } else {
                                appLogger.setLogMessage('API_OK').botMessage(botIcons.API_OK);
                            }
                        }
                    }

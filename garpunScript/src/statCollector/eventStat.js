...
const { CamerasWatcherProcessManager } = require('../camerasWatcherProcess/ProcessManager');
...
class GetEventsStat {
    ...
    getStat() {
        ...
        if (checkResult.isDataOk) {
            ...
            if (this.cameraName && typeof this.cameraName === 'string') {
                ...
                camerasList = CameraModel.findAll({
                    raw: true,
                    where: {
                        // isOnLine: true,
                    },
            ...
            const eventsList = CamEventModel.findAll(eventsDbRequest);
            return Promise.all([camerasList, eventsList]).then((result) => {
                const [camerasList, eventsList] = result;
                const { apiResTime, eventsByTime, eventsErrorsStat } =                    this.getStatFormEventsArray(eventsList);
                ...
                camerasList.forEach((camera) => {
                    ...
                    const cameraEventsStat =                        this.getStatFormEventsArray(eventsByCameraArray);
    ...
    static getLastCamsEventsTime() {
        return new Promise((resolve) => {
            const camerasWatcher =  CamerasWatcherProcessManager.getInstance();
    ...
    static async createStatMessage(dataArray, messageHeader, chartType = null) {
        ...
        if (dataKeys.length > 0) {
            ...
            dataKeys.forEach((keyName) => {
                ...
                if (value > 0) {
                    const keyNameForMsg = reportRowsNames[keyName] || keyName;
            ...
            if (chartType) {
                ...
                try {
                    ...
                    appLogger.setLogMessage(new AppError(error, 'TASK_SCHEDULE')).error().toErrorLog();
                }
